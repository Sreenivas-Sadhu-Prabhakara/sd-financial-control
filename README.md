...
| **Asset Type** | Financial Control Record |
| **Control Record** | Financial Control Record Management Plan |
...
| POST | `/v1/financial-control-record-management-plan/initiate` | Initiate |
| GET | `/v1/financial-control-record-management-plan` | Retrieve (list) |
| GET | `/v1/financial-control-record-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/financial-control-record-management-plan/{crId}/update` | Update |
| PUT | `/v1/financial-control-record-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/financial-control-record-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-financial-control:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-financial-control ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-financial-control` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
